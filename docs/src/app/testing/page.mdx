...
5. Verifies overlay WebView create, resize, navigate, and close commands
6. Verifies the responses

The CEF smoke step additionally requires a local CEF layout or `-Dcef-auto-install=true`; it exercises `native.ping` and overlay WebView create/resize/navigate/close through the automation bridge. These steps are intentionally opt-in because they need a GUI-capable macOS session.
