...
The native-shell smoke step launches `examples/native-shell`, verifies toolbar/sidebar/statusbar/WebView rows in the automation snapshot, dispatches `app.refresh` through the bridge, and checks that the native status label reflects the command source.
