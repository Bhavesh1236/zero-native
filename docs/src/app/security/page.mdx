...
`js_window_api` exposes the JavaScript command, window, view, WebView, and platform support helpers, but it does not bypass security. Command routing (`zero-native.command.invoke`), platform support queries (`zero-native.platform.supports`), window commands (`zero-native.window.list`, `create`, `focus`, `close`), view commands (`zero-native.view.create`, `list`, `update`, `setFrame`, `setVisible`, `focus`, `close`), and WebView commands (`zero-native.webview.create`, `list`, `setFrame`, `navigate`, `setZoom`, `setLayer`, `close`) must come from an allowed origin and must have the `window` permission when runtime permissions are configured. View and WebView commands can only target the window that sent the bridge message. WebView URLs must also be allowed by `security.navigation.allowed_origins`, and child WebViews receive `window.zero` only when created with `bridge: true`.
